...
const register = async (req, res) => {
    const { name, username, email, password, confPassword, bio, status } = req.body
    ...
    try {
        ...
        await query("INSERT INTO userS(name, username, email, password, bio, status) values (?,?,?,?,?,?)", [name, username, email, hash, bio, status])
...
const login = async (req, res) => {
    ...
    try {
        ...
        const [check] = await query("select id, email, password, status from users where id=?", [validation.id])
...
const loginstatus = async (req, res) => {
    const { email, password: inputPass, status } = req.body
    // TODO Validasi email dan password
    try {
        const [validation] = await query("select id from users where email=? AND status = ?", [email, status])
